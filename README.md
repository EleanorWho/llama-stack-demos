...
A collection of hands-on demos for [OGX](https://github.com/ogx-ai/ogx), covering everything from basic chat completions to RAG, agents, observability, and end-to-end applications. All demos run locally in your terminal.
...
These demos use [Ollama](https://ollama.com/) as the default model backend, but OGX supports many others (vLLM, Together, Fireworks, OpenAI, etc.). See the [OGX documentation](https://github.com/ogx-ai/ogx) for the full list of supported providers.
...
If you want to deploy OGX on Kubernetes instead of running locally, see [`deployment/kubernetes/`](deployment/kubernetes/) for manifests and instructions covering vLLM, the OGX Kubernetes operator, and MCP servers on a Kind cluster.
